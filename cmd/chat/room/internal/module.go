...
func (m *Module) checkDestroyRoom() {
	...
	if len(destroyRooms) > 0 {
		...
		cluster.Go("gateway", "DestroyRoom", destroyRooms)
...
func (m *Module) NewRoom(name string) *RoomInfo {
	...
	room.module = m
	room.userServerMap = map[int64]string{}
...
type RoomInfo struct {
	...
	startNullTime int64
	msgList       []*pb.ChatMsg
	userServerMap map[int64]string
}
...
func (r *RoomInfo) IsInRoom(userId int64) bool {
	_, ok := r.userServerMap[userId]
	return ok
}

func (r *RoomInfo) EnterRoom(userId int64, serverName string) bool {
...
func (r *RoomInfo) LeaveRoom(userId int64) bool {
	ok := r.IsInRoom(userId)
	if ok {
...
func (r *RoomInfo) GetMsgList() []*pb.ChatMsg {
	return r.msgList
}

func (r *RoomInfo) SendMsg(userId int64, msgContent []byte) error {
	if !r.IsInRoom(userId) {
		return errors.New("you have not in room")
	}

	msg := &pb.ChatMsg{RoomName: r.name, UserId: userId, MsgTime: time.Now().Unix(), MsgContent: msgContent}
	...
	serverUserMap := map[string][]int64{}
	for userId, serverName := range r.userServerMap {
		if userIds, ok := serverUserMap[serverName]; ok {
			...
			serverUserMap[serverName] = []int64{userId}
		}
	}
